...
test.describe("On-chain store checkout (testnet USDC)", () => {
  ...
  test("fake Google + real testnet treasury: pay checkout, recon records the hash", async ({
    request,
  }) => {
    ...
    const charge = await request.post("/api/checkout/create", {
      data: { amountUsd: "1", reference: `e2e-${subject}` },
    });
    ...
    const amountUsd = chargeBody.amountUsd ?? "1";
    ...
    const summary = (await recon.json()) as {
      ...
      charges?: Array<{ id: string; stellarTxHash: string | null }>;
    };
