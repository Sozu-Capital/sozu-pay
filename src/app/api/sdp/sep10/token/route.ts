...
export async function POST(request: NextRequest) {
  ...
  const networkPassphrase =
    typeof body.network_passphrase === "string"
      ? body.network_passphrase.trim()
      : "";
  const serverAccountId =
    typeof body.server_account_id === "string"
      ? body.server_account_id.trim()
      : "";
  const webAuthDomain =
    typeof body.web_auth_domain === "string"
      ? body.web_auth_domain.trim()
      : "";
  const homeDomains = Array.isArray(body.home_domains)
    ? (body.home_domains as unknown[]).filter(
  ...
  if (
    ...
    return NextResponse.json(
      {
        error:
          "Missing transaction_xdr, network_passphrase, server_account_id, web_auth_domain, or home_domains",
      },
      { status: 400 }
    );
  }
