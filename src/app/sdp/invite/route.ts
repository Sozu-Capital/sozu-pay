...
function htmlError(title: string, message: string, status: number) {
  const body = `<!DOCTYPE html><html><head><meta charset="utf-8"/><title>${title}</title></head><body style="font-family:system-ui;padding:2rem;max-width:36rem"><h1>${title}</h1><p>${message}</p></body></html>`;
...
export async function GET(request: NextRequest) {
  ...
  if (allowed.length === 0) {
    return htmlError(
      ...
      "SDP_ALLOWED_DOMAINS is not set. Add comma-separated SDP hostnames (e.g. sdp.example.com) before accepting invitations.",
      503
    );
  }
  ...
  const pathname =
    process.env.SDP_INVITE_PATHNAME?.trim() || "/sdp/invite";
