...
describe("getCheckoutBaseUrl", () => {
  it("uses the SozuPay app URL, not SozuCredit", () => {
    ...
    const prevCredit = process.env.NEXT_PUBLIC_SOZUCREDIT_URL;
    const prevCreditServer = process.env.SOZUCREDIT_URL;
    process.env.NEXT_PUBLIC_APP_URL = "https://pay.sozu.capital";
    process.env.NEXT_PUBLIC_SOZUCREDIT_URL = "https://credit.sozu.capital";
    ...
    try {
      ...
      if (prevCredit === undefined) delete process.env.NEXT_PUBLIC_SOZUCREDIT_URL;
      else process.env.NEXT_PUBLIC_SOZUCREDIT_URL = prevCredit;
