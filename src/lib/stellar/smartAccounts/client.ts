...
async function loadConfig(): Promise<SmartAccountKitConfigResponse> {
  if (_cfg) return _cfg;
  const res = await fetch("/api/smart-accounts/config", { credentials: "include" });
  const data = (await res.json().catch(() => ({}))) as Partial<SmartAccountKitConfigResponse>;
  if (!res.ok) {
    throw new Error((data as { error?: string }).error ?? "Failed to load smart account config");
  }
